...
public class ArActivity extends ActionBarActivity implements SensorEventListener{
...
//		getWindow().addFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
		...
		getWindowManager().getDefaultDisplay().getMetrics(displayMetrics);
	     ...
	     actionBar.setTitle("AR View");
	       
	     Bundle bundle = getIntent().getExtras();
		 type = bundle.getInt(getString(R.string.key_type_search));
			
		screenHeight = displayMetrics.heightPixels;
		screenWidth = displayMetrics.widthPixels;

		upperLayerLayout = new RelativeLayout(ArActivity.this);
		...
		_context = this;
		cameraView = new CameraView(ArActivity.this);
		radarMarkerView = new RadarMarkerView(ArActivity.this, displayMetrics, upperLayerLayout);

		displayMetrics = new DisplayMetrics();
		getWindowManager().getDefaultDisplay().getMetrics(displayMetrics);

		RelativeLayout headerRelativeLayout = new RelativeLayout(ArActivity.this);
		...
		if(!isInited){
			dataView = new DataView(ArActivity.this);
...
//							Toast.makeText(_context, "match Found its "+dataView.places[i], Toast.LENGTH_SHORT).show();
	...
	public void onSensorChanged(SensorEvent evt) {
		...
		if (gravSensorVals != null && magSensorVals != null) {
			...
			ArActivity.azimuth = (float)(((results[0]*180)/Math.PI)+180);
			ArActivity.pitch = (float)(((results[1]*180/Math.PI))+90);
			ArActivity.roll = (float)(((results[2]*180/Math.PI)));
...
class RadarMarkerView extends View{
	...
	protected void onDraw(Canvas canvas) {
		super.onDraw(canvas);
		ArActivity.paintScreen.setWidth(canvas.getWidth());
		ArActivity.paintScreen.setHeight(canvas.getHeight());
		ArActivity.paintScreen.setCanvas(canvas);


		if (!ArActivity.dataView.isInited()) {
			ArActivity.dataView.init(ArActivity.type,ArActivity.getContext(),ArActivity.paintScreen.getWidth(), ArActivity.paintScreen.getHeight(),arView.camera, displayMetrics,upperLayoutView);
		}

		ArActivity.dataView.draw(ArActivity.paintScreen, ArActivity.azimuth, ArActivity.pitch, ArActivity.roll);
	}
}
